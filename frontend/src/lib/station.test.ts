...
describe('activePresetIndex', () => {
  ...
  it('prefers an exact TuneIn-id match', () => {
    expect(activePresetIndex(presets, { name: 'whatever', tuneInId: 's3' } as PlayerStation)).toBe(3);
  });
  ...
  it('prefers an exact name over a looser one (no double highlight)', () => {
    ...
    expect(activePresetIndex(presets, { name: 'NPO Radio 1', tuneInId: null } as PlayerStation)).toBe(1);
  });
  ...
  it('returns -1 when a loose match is ambiguous', () => {
    ...
    expect(activePresetIndex(presets, { name: 'NPO Radio 1 Extra', tuneInId: null } as PlayerStation)).toBe(-1);
  });

  it('returns a unique loose match', () => {
    expect(activePresetIndex(presets, { name: 'Jazz FM London', tuneInId: null } as PlayerStation)).toBe(3);
  });
